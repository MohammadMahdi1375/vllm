...
class ReTraceMemory:
    ...
    def detached(self, *, dtype=torch.float16, device=None):
        """Detach one round and store representations in FP16."""
        return type(self)(
...
def condition_query_block(
    ...
    if memory is None:
        return embeddings
    start = 1
    # DSpARK slot i predicts proposal i (position query_i + 1). Therefore
    # memory i belongs to slot i. Skip memory 0 to preserve the anchor input.
    # Using memory[:-1] for mask slots would introduce a one-token shift.
    if embeddings.shape[1] != memory.draft.shape[1]:
        raise ValueError("Memory width does not match the configured proposal slots")
    masks = ReTraceMemory(
        memory.draft[:, start:], memory.target[:, start:], memory.valid[:, start:]
    )
    conditioned = conditioner(embeddings[:, start:], masks, beta)
...
class ReTraceRequestCache:
    ...
    def inputs(
        ...
        m = torch.zeros(
            n, k, self.hidden_size, device=device, dtype=self.storage_dtype or dtype
        )
